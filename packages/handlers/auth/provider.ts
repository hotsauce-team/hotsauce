...
export class PasswordProvider implements AuthProvider {
  ...
  constructor(options: PasswordProviderOptions) {
    ...
    if (!this.usersTable[this.identityField]) {
      throw new Error(`PasswordProvider: identity field '${this.identityField}' not found on users table`);
    }
    if (!this.usersTable[this.passwordField]) {
      throw new Error(`PasswordProvider: password field '${this.passwordField}' not found on users table`);
    }
    if (!this.usersTable[this.idField]) {
      throw new Error(`PasswordProvider: id field '${this.idField}' not found on users table`);
    }
  }
  ...
  async authenticate(credentials: unknown): Promise<AuthUser | null> {
    ...
    try {
      ...
      if (!identityColumn) {
        throw new Error(`Identity field '${this.identityField}' not found on users table`);
      }
      ...
      return {
        ...
        role: this.roleField ? (user[this.roleField] as string | undefined) : undefined,
      };
    } catch {
  ...
  async parseCredentials(request: Request): Promise<PasswordCredentials | null> {
    try {
