...
Deno.test('signJwt: requires secret of at least 32 chars', async () => {
  ...
  try {
    ...
    assertEquals((e as Error).message, 'JWT secret must be at least 32 characters');
  }
});
...
Deno.test('verifyJwt: returns null for invalid signature', async () => {
  ...
  const verified = await verifyJwt(token, 'different-secret-that-is-32-chars-long');
...
Deno.test('verifyJwt: returns null for null/undefined inputs', async () => {
  ...
  assertEquals(await verifyJwt(undefined as unknown as string, TEST_SECRET), null);
...
Deno.test('createJwtPayload: includes role when provided', () => {
  const payload = createJwtPayload('123', 'admin');
...
Deno.test('createJwtPayload: uses custom maxAge', () => {
  const payload = createJwtPayload('123', undefined, 3600); // 1 hour
...
Deno.test('JWT: full sign and verify cycle', async () => {
  // Create payload
  const payload = createJwtPayload('user-42', 'editor', 3600);
