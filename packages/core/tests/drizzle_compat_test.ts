...
Deno.test('drizzle-compat: PgColumnBuilder class exists and is accessible', async () => {
  ...
  assertExists(PgColumnBuilder.prototype, 'PgColumnBuilder.prototype should exist');
});
...
Deno.test('drizzle-compat: SQLiteColumnBuilder class exists and is accessible', async () => {
  ...
  assertExists(SQLiteColumnBuilder.prototype, 'SQLiteColumnBuilder.prototype should exist');
});
...
Deno.test('drizzle-compat: MySqlColumnBuilder class exists and is accessible', async () => {
  ...
  assertExists(MySqlColumnBuilder.prototype, 'MySqlColumnBuilder.prototype should exist');
});
...
Deno.test('drizzle-compat: column builder has config property', async () => {
  ...
  assertExists((col as any).config, 'Column builder should have config property');
});
...
Deno.test('drizzle-compat: config has expected base properties', async () => {
  ...
  assertEquals(config.name, 'test_column', 'config.name should match column name');
  assertEquals(typeof config.notNull, 'boolean', 'config.notNull should be boolean');
  assertEquals(typeof config.hasDefault, 'boolean', 'config.hasDefault should be boolean');
  assertEquals(typeof config.primaryKey, 'boolean', 'config.primaryKey should be boolean');
});
