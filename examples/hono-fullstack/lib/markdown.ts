 ...
 * Original author: Jason Miller <jason@developit.ca>
 * License: MIT
 *
 * Vendored to avoid supply chain risk from build tooling.
 * Source: https://raw.githubusercontent.com/developit/snarkdown/master/src/index.js
 */
...
export function parseMarkdown(
  md: string,
  prevLinks?: Record<string, string>,
): string {
  const tokenizer =
    /((?:^|\n+)(?:\n---+|\* \*(?:  \*)+)\n)|(?:^``` *(\w*)\n([\s\S]*?)\n```$)|((?:(?:^|\n+)(?:\t|  {2,}).+)+\n*)|((?:(?:^|\n)([>*+-]|\d+\.)\s+.*)+)|(?:!\[([^\]]*?)\]\(([^)]+?)\))|(\[)|(\](?:\(([^)]+?)\))?)|(?:(?:^|\n+)([^\s].*)\n(-{3,}|={3,})(?:\n+|$))|(?:(?:^|\n+)(#{1,6})\s*(.+)(?:\n+|$))|(?:`([^`].*?)`)|(  \n\n*|\n{2,}|__|\*\*|[_*]|~~)/gm;
  ...
  md = md
    .replace(/^\[(.+?)\]:\s*(.+)$/gm, (_s, name: string, url: string) => {
  ...
  while ((token = tokenizer.exec(md))) {
    ...
    if (prev.match(/[^\\](\\\\)*\\$/)) {
      ...
      chunk = `<img src="${encodeAttr(token[8])}" alt="${
        encodeAttr(token[7])
      }">`;
    } // Links:
    else if (token[10]) {
      ...
      chunk = '<' + t + '>' + parseMarkdown(token[12] || token[15], links) +
        '</' + t + '>';
    } // `code`:
    else if (token[16]) {
